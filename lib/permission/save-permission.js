...
function parseArgs(args) {
  if (args.length < 2) {
    warn('用法: openyida save-permission <appType> <formUuid> [--create --name <名称>] [--data-permission <json>] [--action-permission <json>] [--members <userIds>]');
  ...
  for (let index = 2; index < args.length; index++) {
    if (args[index] === '--create') {
      ...
      warn('⚠️  字段权限（--field-permission）暂不支持，请通过宜搭管理后台手动操作。');
      process.exit(1);
    }
  }
  ...
  if (!createMode && !dataPermission && !actionPermission && !members) {
    warn('❌ 请至少提供 --data-permission、--action-permission 或 --members 参数之一');
    process.exit(1);
  }
