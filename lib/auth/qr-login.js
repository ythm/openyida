...
async function renderQrCodeInTerminal(url) {
  try {
    ...
    if (qrcode) {
      ...
      // 降级：输出 URL 提示用户手动访问
      warn(t('qr_login.qrcode_fallback'));
      warn(`  ${url}`);
    }
  } catch (err) {
    warn(t('qr_login.qrcode_render_failed', err.message));
    warn(`  ${url}`);
  }
}
...
async function qrLogin(options = {}) {
  ...
  const { c: qc, banner: qBanner, step: qStep, info: qInfo, success: qSuccess, warn: qWarn, label: qLabel, sep: qSep } = require('../core/chalk');
